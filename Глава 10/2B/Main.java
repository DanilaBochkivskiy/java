//Выполнить задания из варианта B гл. 4, сохраняя объекты приложения в одном или нескольких
// файлах с применением механизма сериализации.
// Объекты могут содержать поля, помеченные как static, а также transient.
// Для изменения информации и извлечения информации в файле создать специальный класс-коннектор
...
public class Main {
    public static void main(String[] args) {
        ...
        GiftConnector connector = new GiftConnector("gift.dat");
        ...
        while (true) {
            ...
            System.out.println("7. Сохранить подарок (Сериализация)");
            System.out.println("8. Загрузить подарок (Десериализация)");
            ...
            switch (choice) {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    connector.saveGift(gift);
                    break;
                case 8:
                    Sweet.Address = "ИЗМЕНЕННЫЙ СКЛАД";
                    Gift loaded = connector.loadGift();
                    if (loaded != null) {
                        gift = loaded;
